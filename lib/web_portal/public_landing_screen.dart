      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const _HeroSection(),
            const _StatsSection(),
            const _CoordinatorsPublicSection(),
...
/// قسم "منسّقو الأقسام" في الصفحة الرئيسية العامة - عرض فقط (بلا تسجيل
/// دخول)، يُظهر اسم وبريد كل منسّق/ة رسميًا للتواصل المباشر. البيانات تُقرأ
/// لحظيًا من Firestore (coordinator_contacts) ولا يمكن تعديلها إلا من لوحة
/// إدارة الوحدة (حساب admin/salfawaz)، عبر شاشة CoordinatorsContactsScreen.
class _CoordinatorsPublicSection extends StatelessWidget {
  const _CoordinatorsPublicSection();
        ...
        child: ConstrainedBox(
          ...
          child: Column(
            ...
            children: [
              const _SectionHeader(
                eyebrow: 'تواصل مباشر',
                title: 'منسّقو الأقسام',
                icon: Icons.support_agent_outlined,
              ),
              const SizedBox(height: 8),
              Text(
                'للتواصل الرسمي المباشر بخصوص طلبات قسمك وشطرك',
                style: TextStyle(color: Colors.grey.shade600, fontSize: 12.5),
              ),
              const SizedBox(height: 28),
              StreamBuilder<Map<String, Coordinator>>(
                stream: FirestoreCoordinatorService.watchAll(),
                builder: (context, snapshot) {
                  ...
                  // ترتيب الأقسام المعتمد (نفس ترتيب ExcelParserService.departments
                  // المستخدَم في كل مكان آخر بالتطبيق)، وتحت كل قسم شطر الطلاب ثم
                  // شطر الطالبات.
                  final departmentBlocks = ExcelParserService.departments.map((department) {
                    final male = saved['${ExcelParserService.shatrMale}|$department'] ??
                        Coordinator(shatr: ExcelParserService.shatrMale, department: department, name: '', email: '');
                    final female = saved['${ExcelParserService.shatrFemale}|$department'] ??
                        Coordinator(shatr: ExcelParserService.shatrFemale, department: department, name: '', email: '');
                    return (department: department, male: male, female: female);
                  }).where((b) =>
                      b.male.name.isNotEmpty || b.male.email.isNotEmpty ||
                      b.female.name.isNotEmpty || b.female.email.isNotEmpty).toList();

                  if (departmentBlocks.isEmpty) {
                    return Padding(
                      padding: const EdgeInsets.symmetric(vertical: 12),
                      child: Text(
                        'ستظهر بيانات منسّقي الأقسام هنا فور اعتمادها.',
                        textAlign: TextAlign.center,
                      ...
                      for (var i = 0; i < cards.length; i++) ...[
                        ...
                        if (i < cards.length - 1)
                          const Padding(
                            ...
                            child: _FlowArrow(vertical: true),
                          ),
                      ],
                    ],
                  );
...
/// سهم اتصال صغير بين بطاقتي قسمين متتاليين - يعطي انطباع "مخطط متسلسل"
/// بدل بطاقات منفصلة عشوائيًا، بلون الهوية الذهبي.
class _FlowArrow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Icon(
      vertical ? Icons.arrow_downward_rounded : Icons.arrow_back_rounded,
      size: 20,
...
/// بطاقة قسم واحد ضمن قسم "منسّقو الأقسام" بالصفحة الرئيسية - شريط علوي
/// بهوية الوحدة البصرية (أخضر/ذهبي) باسم القسم، ثم عمودا الشطرين (طلاب/طالبات)
/// جنبًا إلى جنب، كل عمود يعرض الاسم والبريد معًا (البريد أسفل الاسم مباشرة).
class _DepartmentCoordinatorsCard extends StatelessWidget {
  const _DepartmentCoordinatorsCard({
  ...
  final String department;
  final Coordinator male;
  final Coordinator female;
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Column(
              children: [
                const Icon(Icons.apartment_rounded, color: AppColors.goldLight, size: 20),
                const SizedBox(height: 6),
                Text(
                  department,
                  textAlign: TextAlign.center,
          ...
          IntrinsicHeight(
            child: Row(
              ...
              children: [
                Expanded(
                  child: _CoordinatorColumn(
                    shatr: ExcelParserService.shatrMale,
                    coordinator: male,
                    isFemale: false,
                  ),
                ),
                ...
                Expanded(
                  child: _CoordinatorColumn(
                    shatr: ExcelParserService.shatrFemale,
                    coordinator: female,
                    isFemale: true,
                  ),
                ),
...
/// عمود شطر واحد (طلاب/طالبات) داخل بطاقة القسم - يعرض شارة الشطر، ثم اسم
/// المنسّق/ة، ثم بريده الرسمي مباشرة أسفل الاسم مع أيقونة بريد صغيرة.
class _CoordinatorColumn extends StatelessWidget {
  const _CoordinatorColumn({
    required this.shatr,
  ...
  @override
  Widget build(BuildContext context) {
    final hasName = coordinator.name.isNotEmpty;
    return Padding(
        ...
        children: [
          ...
          CircleAvatar(
            ...
            child: Icon(
              ...
              size: 15,
              color: hasName ? AppColors.green : Colors.grey.shade400,
            ),
          ),
          const SizedBox(height: 8),
          Text(
            hasName ? displayName(coordinator.name) : 'لم يُحدَّد بعد',
            textAlign: TextAlign.center,
            softWrap: true,
            style: TextStyle(
              fontWeight: FontWeight.bold,
          ...
          if (coordinator.email.isNotEmpty) ...[
            const SizedBox(height: 8),
            InkWell(
              onTap: () => openMailto(coordinator.email),
              ...
              child: Padding(
                ...
                child: Row(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Icon(Icons.email_outlined, size: 12, color: AppColors.green),
                    const SizedBox(width: 4),
                    Flexible(
                      child: Text(
                        coordinator.email,

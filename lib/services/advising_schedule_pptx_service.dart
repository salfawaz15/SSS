...
class AdvisingSchedulePptxService {
  ...
  static String _slideXml({
    ...
    //
    // صندوق الشعار **يجب** أن يطابق نسبة أبعاد unit_logo_final.png الحقيقية
    // (378×124 بكسل، مستطيل عريض ~3:1) - صندوق مربَّع (كما كان: 520000×520000)
    // يجبر PowerPoint على ضغط/تمديد الصورة داخله فيظهر الشعار مشوَّهًا (مضغوطًا
    // أفقيًا) عند فتح الملف مباشرة، رغم أن `noChangeAspect="1"` يمنع صحيح تغيير
    // النسبة *لاحقًا* عند السحب يدويًا لكنه لا يصحح الصندوق الأولي نفسه - دليل
    // فعلي من سليمان (2026-08-25): "يظهر الشعار هكذا [مضغوطًا]... اقوم انا
    // بالتعديل عليه واسحبه قليلا لكي يكون واضح" بكل ملف بوربوينت يُنزَّل.
    // العرض يبقى كالسابق (520000) فلا حاجة لتحريك النص المجاور، والارتفاع
    // يُشتق من نسبة الصورة الحقيقية (520000 × 124/378) ثم يُتمركز رأسيًا داخل
    // الشريط العلوي (headerH = 750000).
    // كبَّرتُ الشعار بوضوح - سليمان صراحةً (2026-08-25): "الصورة ضبطت [نسبة
    // الأبعاد صحيحة الآن] ولكن حجمها صغير جدًا جدًا". العرض الآن ~1.1 بوصة
    // (كان ~0.57) مع إبقاء نسبة الصورة الحقيقية (378×124) فلا تشوّه إطلاقًا.
    const logoCx = 1000000;
    const logoCy = (logoCx * 124) ~/ 378;
    shapes.write(_pictureLogo(
      x: _marginX + 120000,
      y: (headerH - logoCy) ~/ 2,
      cx: logoCx,
      cy: logoCy,
    ));
    const titleX = _marginX + 1200000;
    shapes.write(_textBox(
      x: titleX,
      y: 90000,
      cx: _contentW - (titleX - _marginX),
      cy: 340000,

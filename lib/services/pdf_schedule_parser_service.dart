  ...
  static List<ParsedCourseSectionWithShatr> parseSectionsWithShatr(List<int> pdfBytes) {
    ...
    for (final cells in rows) {
      ...
      if (isMain) {
        ...
        final from = at(10);
        final to = at(11);
        // خلية "القاعة" نفسها قد تختفي كليًا من الاستخراج حين تكون فارغة
        // أصلًا بمصدر PDF (مقرر بلا قاعة مخصَّصة) - تمامًا كخلية "المستفيد"
        // الموثَّقة أدناه، لكن هنا اختفاء القاعة يُزيح خلية المحاضر (الأقرب
        // لها من الجهة الأخرى) لتُلتقَط خطأً كقاعة بدل النص الحقيقي (دليل
        // فعلي من سليمان 2026-09-11: مقرر "التأمين التعاوني" 604466 أظهر اسم
        // محاضر كامل بخانة القاعة). `_looksLikeRoom` (موثَّقة أعلاه، مستخدَمة
        // أصلاً بصفوف المتابعة) تكشف هذه الحالة: نص القاعة الحقيقي رقم أو من
        // مفردات القاعات المعروفة، لا اسمًا عربيًا متعدد الكلمات.
        final roomCandidate = at(12);
        final roomMissing = !_looksLikeRoom(roomCandidate);
        final room = roomMissing ? '' : _normalizeRoom(roomCandidate);
        ...
        // بلوحة PDF رغم وجوده صراحةً بلوحة Word المقابلة). إن غابت القاعة
        // (`roomMissing`)، تُسترجَع الخلية المسروقة بتوسيع الحد بمقدار واحد.
        final searchLimit = roomMissing ? n - 11 : n - 12;

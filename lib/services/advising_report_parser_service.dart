...
class AdvisingReportParserService {
  ...
  /// يُصلح حالة انقسام خلية "الاسم" الفعلي لعدة خلايا متجاورة (خليتين أو
  /// أكثر) - اكتُشفت فعليًا (سليمان 2026-08-25) بفارق 25 طالبة عن ملف "بيانات
  /// الطلبة" اليدوي المرجعي، كلها بنفس النمط تحديدًا: فجوة أفقية غير معتادة
  /// بين أجزاء الاسم أثناء استخراج PDF (مثال خام حقيقي: `[الإدارة, المالكي,
  /// انتصار منصور بن محمد, 44450079, 8]` بدل 4 خلايا متوقَّعة) تُزيح كل
  /// الأعمدة اللاحقة لعمود الاسم. **مُعمَّم لاحقًا (2026-08-26)**: النسخة
  /// الأولى افترضت انقسامًا لخليتين فقط بالضبط (`row.length ==
  /// headerColumnCount + 1`) - سبع حالات فعلية (نفس الجلسة، تحقَّق منها
  /// سليمان بملف مرجعي آخر) بقيت غير مُصلَحة رغم تطابق نمطها تمامًا مع
  /// الحالات المُصلَحة، لأن الاسم انقسم فيها لعدد خلايا مختلف (متغيّر حسب طول
  /// كل اسم فعليًا، لا ثابت خليتين دومًا). الآن: يُدمَج **كل** ما بين [nameCol]
  /// وأول خلية تالية تبدو رقمًا جامعيًا حقيقيًا (٦ أرقام فأكثر - [idCol] نفسه
  /// بعد الانزياح)، بأي عدد فعلي من الخلايا. عكس [_splitMergedNameAndDepartment]
  /// (تلك تُصلح اندماج خليتين في واحدة، هذه تُصلح انقسام خلية واحدة لعدة).
  /// null لو لم تنطبق شروط الإصلاح الآمنة (لا رقم جامعي بعد [nameCol]، أو
  /// الرقم في [nameCol] نفسه أصلًا - لا انقسام فعليًا).
  static List<String>? _repairSplitNameCell(List<String> row, int nameCol) {
    if (nameCol < 0 || nameCol >= row.length) return null;
    var idFragmentIndex = -1;
    for (var i = nameCol; i < row.length; i++) {
      if (RegExp(r'^\d{6,}$').hasMatch(row[i].trim())) {
        idFragmentIndex = i;
        break;
      }
    }
    // لا رقم جامعي موجود بعد عمود الاسم إطلاقًا، أو الرقم مباشرة في مكانه
    // المتوقَّع أصلًا (لا انقسام حقيقي) - لا شيء يُصلَح.
    if (idFragmentIndex <= nameCol) return null;

    // خلية واحدة فقط بين [nameCol] والرقم الجامعي يعني عمود الاسم سليمًا
    // أصلًا (لا انقسام) - حتى لو كان الصف أطول من المتوقَّع لسبب آخر (عمود
    // إضافي غير متعلق بالاسم إطلاقًا)، لا يوجد ما يُصلَح هنا تحديدًا.
    if (idFragmentIndex - nameCol < 2) return null;

    final fragments = row.sublist(nameCol, idFragmentIndex).map((f) => f.trim()).toList();
    if (fragments.any((f) => f.isEmpty)) return null;

    // ترتيب الخلايا وقت الاستخراج هو ترتيب أفقي (يسار ← يمين) لموضعها على
    // الصفحة، بعكس ترتيب قراءة الاسم العربي الكامل (اسم الشخص ثم الأب
    // فالجد يُقرأ أولًا، واسم العائلة/القبيلة يُقرأ أخيرًا رغم ظهوره أول خلية
    // هنا لأنه يقع يسار بقية الاسم على الصفحة) - يُعكَس ترتيب الخلايا هنا
    // ليطابق ترتيب القراءة الصحيح (تحقَّقتُ من أسماء حقيقية معروفة سليمان
    // 2026-08-25: "المالكي" + "انتصار منصور بن محمد" ← "انتصار منصور بن محمد
    // المالكي"، لا العكس).
    final mergedName = fragments.reversed.join(' ');
    return [...row.sublist(0, nameCol), mergedName, ...row.sublist(idFragmentIndex)];
  }
  ...
  static List<AdvisingCaseRecord> parseRows(
    ...
    for (final rawRow in rows) {
      ...
      if (headerColumnCount > 0 && row.length > headerColumnCount) {
